...
async function raw(input) {
  ...
  return { data, width, height, alpha: (x, y) => data[(y * width + x) * channels + 3] };
}
...
async function main() {
  ...
  for (const { letter, buffer } of masters) {
    ...
    /* Where a hand would write on this folder: the middle of the blue paper
       between the bottom of the photo opening and the bottom of the folder.
       The four templates put their openings at different heights, so a shared
       constant would push the label onto the frame of one and off the paper
       of another. */
    const labelMid = pct((hole.maxY + bounds.maxY) / 2, height);
    ...
    rows.push({ letter, imageInset, tabBox, labelMid });
  }
